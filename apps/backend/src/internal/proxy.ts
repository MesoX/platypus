 ...
 *
 * Trusted services on the same network (e.g. sandbox containers) call this
 * endpoint to fetch raw resource bytes from a workspace's external
 * integration (Google Drive today; expandable in the future).
 * The proxy uses the workspace's stored OAuth credentials, refreshes
 * them if expired, and streams the upstream response back to the caller.
 * Tokens never leave the backend.
 *
 * Auth: shared `INTERNAL_SECRET` env var (Bearer header).
 */
