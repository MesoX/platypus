...
import { type Schedule, type Agent } from "@platypus/schemas";
...
const getBrowserTimezone = (): string => {
  try {
    ...
    // Return browser timezone if it's in our list, otherwise fallback to UTC
...
const ScheduleForm = ({
  ...
  const { data: schedule, isLoading: scheduleLoading } = useSWR<Schedule>(
    scheduleId && user
      ? joinUrl(
          backendUrl,
  ...
  // Initialize form with existing schedule data when editing
  useEffect(() => {
    if (schedule) {
      setFormData({
        name: schedule.name,
  ...
  if (agentsLoading || (scheduleId && scheduleLoading)) {
        ...
        <FieldGroup>
          ...
          {scheduleMode === "simple" && (
            <>
              ...
              {/* Time selectors (not shown for minute-based intervals) */}
